...
- Governance/workflow assets are now part of the foundation scaffold baseline: fresh `nurt new` output writes bundled copies of `btca.config.jsonc`, `AGENTS.md`, `PLAN.md`, `README.md`, `README.BMAD-GUIDE.md`, `README.RALPH.md`, scaffold-stub `PROGRESS.md`, `scripts/{RALPH.sh,configure-repo-protections.sh,synthetic-quotas.sh,task-template-schema.json,validate_template.py,visualize_plan.py}`, `docs/{archive,archive/plans,archive/progress,session-summaries,tasks,workflows}`, `.agent`, and `.opencode/command`, while template sync remains available for maintainer update flows.
...
- Root `PROGRESS.template.md` is preserved as the snapshot-builder source for scaffolded `foundation/PROGRESS.md`, separating generated-project governance stubs from the template repo's live cumulative `PROGRESS.md` tracker.
