...
- Repository hardening automation now includes `.template_scripts/configure-repo-protections.sh`, which applies branch-protection baseline controls (PR + required checks + linear history + conversation resolution + no force-push/delete) and enables repository-level Dependabot security updates.
