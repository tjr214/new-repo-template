...
- Snapshot maintenance command path is implemented at `nurt template-assets snapshot`, which validates the manifest-backed bundled template set and refreshes snapshot metadata via metadata hashing instead of serving as the primary template-authoring path.
