...
- The foundation `.opencode/command` baseline now explicitly includes `repo-gh-make-n-merge-PR.md` in both scaffold output and the packaged snapshot runtime manifest, keeping runtime asset discovery aligned with the manifest-driven source bundle.
...
- Snapshot maintenance command path is implemented at `nurt template-assets validate`, which validates the manifest-backed bundled template set and refreshes snapshot metadata via metadata hashing instead of serving as the primary template-authoring path.
...
- `tests/contracts/test_root_workspace_contract.py`
  - Contract intent: root workspace config files (`package.json`, `turbo.json`) are present in dry-run/scaffold output, include baseline cross-platform script/task wiring for `dev`, `build`, `test`, `lint`, and `typecheck`, and mirror the expanded foundation governance/agent asset baseline (`btca.config.jsonc`, `AGENTS.md`, template-backed root `PLAN.md`/`PROGRESS.md`, `README*`, helper scripts, `.github/workflows/*`, `docs/{ARCHITECTURE.md,LIVING_DOCS.md,archive/{plans,progress},markdown-templates,tasks,workflows}`, `.agent`, and `.opencode/command`).
...
- `tests/contracts/test_snapshot_assets_contract.py`
  - Contract intent: packaged snapshot templates are loadable, metadata generation remains deterministic, and the packaged runtime manifest stays aligned with source-manifest foundation `.opencode/command` entries.
