...
- Successful non-dry-run `nurt new` completion now ends with a Rich `Setup Complete` overview from `src/new_repo_template/post_create.py`, summarizing project context plus post-create accomplishments and explicitly calling out the project-directory handoff.
- After rendering that overview, `src/new_repo_template/nurt_cli.py` now changes the CLI process working directory to the generated project path before returning control to the caller.
