...
- The next implementation slice is feature `5.0` for `nurt sync template-assets`: replace the placeholder native sync path with a manifest-driven implementation and remove the remaining legacy updater after manual review.
- Feature `5.0` planning is now locked: `nurt sync template-assets` will validate a real nurt repo via `.nurt/repo.json`, require a clean git working tree for non-dry-run execution, and sync only explicit managed files while leaving custom sibling files and unrelated repo content untouched.
- The same planning pass fixed the source-of-truth model for that slice: `src/new_repo_template/snapshot_assets/source_manifest.json` remains the single manifest, and per-entry `management` metadata will distinguish scaffold-only versus scaffold-plus-sync assets without introducing a second manifest.
- Template-asset sync sourcing is now intentionally version-aligned to the installed CLI: feature `5.0` will copy from the bundled snapshot assets shipped with the currently installed `nurt` rather than cloning template-repo HEAD at sync time.
