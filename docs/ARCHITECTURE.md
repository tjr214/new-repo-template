...
- The next React UI architecture direction is now explicit: `shadcn/ui` is the default web component foundation, shared design tokens are intended to carry cross-target visual consistency, mobile/TV remain native presentation surfaces, and desktop is expected to join the shared React model only after the Electron renderer is upgraded from plain DOM scripting to a real React renderer.
- The intended desktop React architecture is now also explicit: the upgraded Electron lane should use a normal `Electron Forge + Vite + React` renderer stack with `@tanstack/react-router` plus `createHashHistory()` as the default client-side routing layer, while `TanStack Start` remains reserved for the web full-stack lane rather than the desktop renderer.
- Shared React architecture is now constrained by a product rule: the common layer should centralize the design language strongly enough that the app still feels like the same product across web, desktop, mobile, and TV, but should not erase platform-specific rendering models, interaction idioms, or native affordances.
- The desktop React migration is intentionally staged: the first renderer implementation only needs a simple hello-world baseline so the target can join the shared React foundation work, while the later `Welcome To Nurt` roadmap item will become the first richer cross-frontend demo surface.
