...
- Planning history now includes the archived root plans at `docs/archive/plans/PLAN_2026-03-19_04-33-30_PM.md` and `docs/archive/plans/PLAN_2026-03-19_12-45-13_PM.md`; features `4.0`, `5.0`, `6.0`, and `7.0` are complete, the active root `PLAN.md` now serves as the completed feature `7.0` implementation record, and the older paired `PLAN.md` / `PROGRESS.md` archive remains at `docs/archive/plans/PLAN_2026-03-08_07-49-04_PM.md` and `docs/archive/plans/PROGRESS_2026-03-08_07-49-04_PM.md`.
...
- The full repository suite is green at `uv run pytest` (219 passed).
...
- `nurt` command bootstrap is implemented at `src/new_repo_template/nurt_cli.py` with command routing (`new`, `add`, `upgrade`, `sync <target>`, `template-assets validate`) and a startup update-check hook that consults `uv tool list --outdated`.
...
- `tests/contracts/test_nurt_cli_contract.py`
  - Contract intent: `nurt` command routing, new-project dry-run parity, repeatable named `--project` support, `nurt add` dry-run/root-validation/foundation-rejection behavior, project-name prompting/normalization when omitted, startup update notice behavior, dry-run safety for `upgrade`/`sync tools`/`sync template-assets`, deterministic non-dry-run failure messaging for native sync paths, and deterministic interactive stdin-failure remediation.
