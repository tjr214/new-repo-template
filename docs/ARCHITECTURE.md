...
- Planning history now includes the newly archived root plan at `docs/archive/plans/PLAN_2026-03-15_12-30-32_PM.md`; root `PLAN.md` now records the active feature `4.0` `nurt add` cycle, and the older paired `PLAN.md` / `PROGRESS.md` archive remains at `docs/archive/plans/PLAN_2026-03-08_07-49-04_PM.md` and `docs/archive/plans/PROGRESS_2026-03-08_07-49-04_PM.md`.
...
- `nurt` command bootstrap is implemented at `src/new_repo_template/nurt_cli.py` with command routing (`new`, `update`, `sync <target>`, `template-assets validate`) and startup update-check hook.
...
- The next implementation slice is feature `4.0` for `nurt add`: existing nurt-generated repos will gain an in-place add workflow with repo-root validation, add-specific CLI/TUI flows, additive-only retrofits for required shared/root workspace wiring, and root lockfile regeneration instead of new-repo post-create behavior.
