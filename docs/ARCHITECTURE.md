...
- Planning history now includes the archived root plan at `docs/archive/plans/PLAN_2026-03-19_12-45-13_PM.md`; feature `4.0` is complete, the active root `PLAN.md` now covers feature `5.0`, and the older paired `PLAN.md` / `PROGRESS.md` archive remains at `docs/archive/plans/PLAN_2026-03-08_07-49-04_PM.md` and `docs/archive/plans/PROGRESS_2026-03-08_07-49-04_PM.md`.
...
- `nurt` command bootstrap is implemented at `src/new_repo_template/nurt_cli.py` with command routing (`new`, `add`, `update`, `sync <target>`, `template-assets validate`) and startup update-check hook.
...
- Governance/workflow assets are now part of the foundation scaffold baseline: fresh `nurt new` output writes bundled copies of `btca.config.jsonc`, `AGENTS.md`, template-backed `PLAN.md` and `PROGRESS.md`, `README.md`, `README.BMAD-GUIDE.md`, `README.RALPH.md`, `scripts/{RALPH.sh,configure-repo-protections.sh,synthetic-quotas.sh,task-template-schema.json,validate_template.py,visualize_plan.py}`, `.github/workflows/{ci.yml,release.yml}`, `docs/{ARCHITECTURE.md,LIVING_DOCS.md,archive,archive/plans,archive/progress,markdown-templates,session-summaries,tasks,workflows}`, `.agent`, and `.opencode/command`, while template sync remains available for maintainer update flows.
...
- The exact-path sync allowlist is intentionally validated against the approved managed inventory, so typo-only or out-of-scope destinations are rejected instead of silently entering the sync surface.
...
- The next implementation slice is feature `6.0`: investigate whether the Python build backend should remain `hatchling` or migrate to `uv build`.
- The eventual self-update UX is now explicitly reserved for feature `7.0` under the command name `nurt upgrade`; any existing `nurt update` scaffolding is transitional and does not define the final product contract.
