...
- The intended desktop React architecture is now also explicit: the upgraded Electron lane should use a normal `Electron Forge + Vite + React` renderer stack with `@tanstack/react-router` as the default client-side routing layer, while `TanStack Start` remains reserved for the web full-stack lane rather than the desktop renderer.
