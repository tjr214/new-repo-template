...
- Installer/tooling migration is now effectively complete: `install.sh` remains a legacy maintainer path with `--dry-run`, but core tool and BMAD updates are now handled by native `nurt tools sync` and `nurt bmad sync` commands rather than shell updater scripts.
...
- Native updater tooling now includes GitHub CLI (`gh`) in the shared `nurt tools sync` runner, including dry-run visibility, live status-table reporting, and install/update handling via platform package managers.
...
- Native OpenCode updater behavior now lives in `src/new_repo_template/tool_sync_runner.py`: existing installs use `opencode upgrade`, missing installs still bootstrap via the installer curl flow, and contract coverage now validates the native `nurt tools sync` path instead of the removed shell script.
- The legacy shell updater files `.template_scripts/update-opencode.sh` and `.template_scripts/update-bmad-method.sh` have been removed; `nurt tools sync` and `nurt bmad sync` are now the only supported update entrypoints for those flows.
