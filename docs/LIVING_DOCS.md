...
- Snapshot asset pipeline is now active: scaffold content is loaded from bundled package templates, `src/new_repo_template/snapshot_assets/templates/` is the canonical template source, and `nurt template-assets validate` is the maintainer validation/metadata-refresh path for the manifest-backed bundle.
