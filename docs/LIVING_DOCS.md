...
- The stale repository-baseline contract drift has now been cleared: tests no longer expect a removed root `install.sh`, the maintainer branch-protection path now targets `nurt secure-repo` instead of a shell wrapper, and README install guidance is validated against a concrete copyable GitHub `uv tool install git+https://github.com/...` command.
...
- The branch-protection baseline is now tuned for the template's real operating mode: `nurt secure-repo` requires PR-based merging and required checks, prompts interactive users for required approvals with default `0`, defaults to `0` automatically under `--no-interactive`, and still allows team repos to opt into `--required-approvals <n>`.
...
- Fresh repos created by `nurt new` now include the foundation governance baseline directly in scaffold output: `btca.config.jsonc`, `AGENTS.md`, template-backed `PLAN.md` and `PROGRESS.md`, the split workflow guides (`README.md`, `README.BMAD-GUIDE.md`, `README.RALPH.md`), `scripts/{RALPH.sh,synthetic-quotas.sh,validate_template.py,visualize_plan.py}`, `docs/{ARCHITECTURE.md,LIVING_DOCS.md,archive,archive/plans,archive/progress,markdown-templates,session-summaries,tasks,workflows}`, `.agent`, and `.opencode/command` mirrored from bundled snapshot assets.
...
- Repository-protection automation is now available via `nurt secure-repo`, with support for dry-run planning, required-check auto-discovery from the latest successful `CI` run, branch-protection enforcement (default branch `main` when omitted), repository auto-detection via `gh repo view` when `--repo` is omitted, repository-level `dependabot_security_updates` enablement, and post-apply verification output.
- The same native command now prompts for required approvals during interactive runs and defaults `required_approving_review_count` to `0` when omitted, preserving PR-only flow for solo repos while still allowing stricter team policies through `--required-approvals`.
