...
- Successful non-dry-run `nurt new` runs now print a Rich `Setup Complete` handoff overview that summarizes the created project, post-create work, optional BMAD/core-tools choices, and closes by explicitly stating that the flow is changing into the new project directory.
- After that completion overview renders, `nurt new` now switches the CLI process working directory to the generated project path before exiting.
