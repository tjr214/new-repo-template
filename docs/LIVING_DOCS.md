...
- Installer/tooling migration is now effectively complete: `install.sh` remains a legacy maintainer path with `--dry-run`, but core tool and BMAD updates are now handled by native `nurt sync tools` and `nurt sync bmad` commands rather than shell updater scripts.
