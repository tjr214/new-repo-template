...
- IMPORTANT: NEVER OVERWRITE A `docs/session-summaries/SESSION_X_SUMMARY.md` file. _Always create a new session summary! The session summary should contain the current date and time in the format `YYYY-MM-DD HH:MM:SS AM/PM`. If you do not have the full date and time, retrieve it with the `date` command._
